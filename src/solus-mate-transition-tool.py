...
translate = _gettext.translation("messages", localedir=localedir)
...
class App():
    ...
    def state_enable_remove(self) -> None:
        ...
        self.builder.get_object("remove_mate").set_tooltip_text(gettext("Uninstall MATE to complete transition"))
    ...
    def startup_checks(self) -> None:
        ...
        if exists == False and self.get_desktop_type() != self.mate_desktop_session:
            ...
            self.on_error_dialog(gettext("Error"), gettext("Logout and login to the MATE session first to continue"))
        ...
        if exists is True and de is not None:
            ...
            if self.get_desktop_type().casefold() == desktop_session.casefold():
                ...
                self.on_error_dialog(gettext("Error"),
                                     gettext("{} is installed but you are not logged into that desktop environment.\nLogout and login to the {} session to continue.").format(pretty_name))
    ...
    def on_success_reboot_dialog(self, de: str, logo: str) -> None:
        ...
        dialog.format_secondary_text(gettext(
    ...
    def on_success_complete_dialog(self, de: str) -> None:
        dialog = Gtk.MessageDialog(
            ...
            text=gettext("Successfully Completed Transition to {}").format(de),
        )
        dialog.format_secondary_text(gettext(
    ...
    def finished(self):
        ...
        self.progress.set_text(gettext("Finished"))
    ...
    def write_temporary_config_files(self, ref: str) -> None:
        try:
            ...
            if 'Authorization failed' in str(e):
                ...
                self.on_error_dialog(gettext("Failed to write temporary config file"), e)
    ...
    def on_refresh_finished(self, source, result, data=None):
        print(gettext("Pkit refreshed cache"))
        self.progress.set_text(gettext("Cache updated"))

    def on_pkit_finished(self, source, result, data=None):
        self.finished()
        print(gettext("Pkit update finished"))
        try:
            results = source.generic_finish(result)
        except Exception as e:
            self.progress.set_text(gettext("Error: {}").format(e))
            print(gettext("Pkit update error:"), e)
    ...
    def pk_resolve_pkgs_async(self, pkgs: list, only_installed: bool, ref: str) -> None:
        ...
        def on_resolve_async(source, result, data=None) -> bool:
            ...
            for i in package_ids:
                ...
                print(gettext("is_installed"), is_installed, name)
                # FIXME: make this not shit
                if data == self.mate_id and not_installed is True:
                    print(gettext("Skipping {}").format(name))
    ...
    def get_desktop_type(self) -> str:
        ...
        print(desktop)
        if desktop is None:
            print(gettext("Warning: XDG_SESSION_DESKTOP is unset!"))
            return ""
        return desktop
    ...
    def read_pkgs_file(self, de: str) -> list:
        ...
        try:
            ...
            self.progress.set_text(gettext("Error: {}").format(e))
            print(gettext("Error Failed to read {}, error: {}").format(path, e))

        if len(contents) == 0:
            self.progress.set_text(gettext("Error: no packages found in {}").format(path))
            print(gettext("Error: No packages found in {}"), path)
